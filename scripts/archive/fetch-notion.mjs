...
// Download media files locally to avoid Notion URL expiration
async function downloadMedia(url, title) {
  ...
  try {
    ...
    const hash = crypto.createHash('md5').update(urlWithoutParams).digest('hex').slice(0, 8);
    const extMatch = urlWithoutParams.match(/\.(jpg|jpeg|png|gif|webp|mp4|mov|mp3|wav|ogg)$/i);
    const ext = extMatch ? extMatch[1].toLowerCase() : 'jpg';
    const filename = `${slugify(title)}-${hash}.${ext}`;
    const localDir = path.join('site', 'media');
    ...
    if (!response.ok) {
      console.warn(`  ↳ Failed to download (${response.status}): ${url.slice(0, 60)}...`);
      return '';
    }
    ...
    console.warn(`  ↳ Error downloading media:`, error.message);
    return '';
  }
}
